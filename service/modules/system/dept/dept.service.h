...
public:
    ...
    cyra::Task<DeptDto> getById(cyra::Context& c, std::int64_t id) {
        ...
        const auto rs = co_await db.query(
            ...
            {cyra::DbValue{id}});
        if (rs.rows().empty()) service::common::throwAppError(DeptError::NOT_FOUND);
    ...
    cyra::Task<void> create(cyra::Context& c, const CreateDeptBody& body) {
        ...
        if (code) {
            const auto exist = co_await db.query(
                ...
                {cyra::DbValue{*code}});
            if (!exist.rows().empty()) service::common::throwAppError(DeptError::CODE_EXISTS);
        }
        if (body.parentId()) {
            ...
            if (parent.rows().empty()) service::common::throwAppError(DeptError::NOT_FOUND);
        }
    ...
    cyra::Task<void> update(cyra::Context& c, std::int64_t id, const UpdateDeptBody& body) {
        ...
        const std::string currentCode =
            rs.rows().front()[1].isNull() ? std::string{} : std::string(rs.rows().front()[1].text());
        ...
        if (body.parentId()) {
            ...
            if (parentId == id) service::common::throwAppError(DeptError::PARENT_SELF);
            if (parentId > 0) {
                const auto parent = co_await db.query(
                    "SELECT id FROM sys_dept WHERE id = ? AND deleted_at IS NULL LIMIT 1",
        ...
        if (code) append("code", cyra::DbValue{*code});
        if (body.parentId()) append("parent_id", cyra::DbValue{static_cast<std::int64_t>(*body.parentId())});
        if (body.sortOrder()) append("`order`", cyra::DbValue{static_cast<std::int64_t>(*body.sortOrder())});
        if (body.leaderId()) append("leader_id", cyra::DbValue{static_cast<std::int64_t>(*body.leaderId())});
        if (body.status()) append("status", cyra::DbValue{std::string(body.status()->view())});

        if (!set.empty()) {
            params.emplace_back(cyra::DbValue{id});
            (void)co_await db.execute("UPDATE sys_dept SET " + set + ", updated_at = NOW() WHERE id = ?",
                                    params);
        }
        co_return;
    }
...
private:
    ...
    static std::vector<DeptRecord> rowsToRecords(const Rows& rows) {
    ...
    static void appendNode(
        cyra::Context& c,
        cyra::List<DeptDto>& out,
        const DeptRecord& record,
    ...
    static cyra::List<DeptDto> buildTree(cyra::Context& c,
                                               const std::vector<DeptRecord>& records) {
        cyra::List<DeptDto> out(c.resource());
