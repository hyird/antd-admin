...
private:
    ...
    cyra::Task<cyra::HttpResponse> tree(cyra::Context& c) {
        co_await service::middleware::requireAnyPermission(c, {"system:menu:query", "system:role:perm"});
        co_return c.json(service::common::ok<MenuListResponse>(
            c,
            co_await menuService().getTree(c, service::common::getQuery(c, "status"))));
    }

    cyra::Task<cyra::HttpResponse> detail(cyra::Context& c) {
        co_await service::middleware::requirePermission(c, "system:menu:query");
        const auto id = service::common::parseIdParam(c);
        co_return c.json(service::common::ok<MenuDetailResponse>(
            c,
            co_await menuService().getDetail(c, id)));
    }
    ...
    cyra::Task<cyra::HttpResponse> batchButtons(cyra::Context& c) {
        ...
        const int created = co_await menuService().batchCreateButtons(
            c,
            c.valid<BatchCreateMenuButtonsBody>());
