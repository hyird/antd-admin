...
class RoleService {
  public:
    ...
    cyra::Task<RolePageDataDto> list(cyra::Context& c, std::int64_t page, std::int64_t pageSize,
  ...
  private:
    ...
    static void setMenuIds(cyra::Context& c, RoleItemDto& item,
    ...
    cyra::Task<void> syncRoleMenus(cyra::Context& c, std::int64_t roleId,
        ...
        for (const auto menuId : *menuIds) {
            (void)co_await db.execute(
                ...
                {cyra::DbValue{roleId}, cyra::DbValue{static_cast<std::int64_t>(menuId)}});
        }
        co_return;
    }
};
