...
struct RoleQuery : service::common::PageParams {
    std::optional<std::string> status;
};

CYRA_MODEL(CreateRoleBody,
    CYRA_FIELD(code, cyra::String,
        CYRA_REQUIRED("角色编码不能为空"),
        CYRA_MIN(1, "角色编码不能为空"),
        CYRA_MAX(50, "角色编码最多50个字符")),
    CYRA_FIELD(name, cyra::String,
        CYRA_REQUIRED("角色名称不能为空"),
        CYRA_MIN(1, "角色名称不能为空"),
...
CYRA_MODEL(UpdateRoleBody,
    CYRA_FIELD(code, cyra::String, CYRA_MAX(50, "角色编码最多50个字符")),
    CYRA_FIELD(name, cyra::String, CYRA_MAX(100, "角色名称最多100个字符")),
    CYRA_FIELD(status, cyra::String),
    CYRA_FIELD_NAME("menu_ids", menuIds, cyra::Array<cyra::Int64>)
);

CYRA_MODEL(RoleItemDto,
...
CYRA_MODEL(RoleOptionDto,
    CYRA_FIELD(id, cyra::Int64),
    CYRA_FIELD(name, cyra::String),
    CYRA_FIELD(code, cyra::String)
);

CYRA_MODEL(RolePageDataDto,
    CYRA_FIELD(list, cyra::List<RoleItemDto>),
