...
struct UserQuery : service::common::PageParams {
    std::optional<std::string> status;
    std::optional<std::int64_t> dept_id;
};

inline bool isPhoneNumber(std::string_view value) {
    if (value.size() != 11 || value[0] != '1' || value[1] < '3' || value[1] > '9') return false;
...
CYRA_MODEL(CreateUserBody,
    ...
    CYRA_FIELD(nickname, cyra::String),
    CYRA_FIELD(phone, cyra::String, CYRA_MATCH("手机号格式不正确", isPhoneNumber)),
    CYRA_FIELD(email, cyra::String, CYRA_EMAIL("邮箱格式不正确")),
    CYRA_FIELD_NAME("dept_id", deptId, cyra::Int64),
    CYRA_FIELD(status, cyra::String),
    CYRA_FIELD_NAME("role_ids", roleIds, cyra::Array<cyra::Int64>,
        CYRA_REQUIRED("至少选择一个角色"),
        CYRA_MIN(1, "至少选择一个角色"))
);

CYRA_MODEL(UpdateUserBody,
    CYRA_FIELD(nickname, cyra::String),
    CYRA_FIELD(phone, cyra::String, CYRA_MATCH("手机号格式不正确", isPhoneNumber)),
    CYRA_FIELD(email, cyra::String, CYRA_EMAIL("邮箱格式不正确")),
    CYRA_FIELD_NAME("dept_id", deptId, cyra::Int64),
...
CYRA_MODEL(UserItemDto,
    ...
    CYRA_FIELD(status, cyra::String),
    CYRA_FIELD(roles, cyra::List<role::RoleOptionDto>)
);

CYRA_MODEL(UserPageDataDto,
    CYRA_FIELD(list, cyra::List<UserItemDto>),
